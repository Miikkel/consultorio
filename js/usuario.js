    ...
    $.ajax({
        ...
        if (resp == 0) {
            ...
            $.ajax({
                ...
                if (resp == 2) {
                    Swal.fire("Mensaje de Advertencia", "Usuario y/o contrase\u00f1a incorrecta, intentos fallidos: " + (resp + 1) + " - Para poder acceder a su cuenta restablecer la contra&#241;a", "warning");
                } else {
                    Swal.fire("Mensaje de Advertencia", "Usuario y/o contrase\u00f1a incorrecta, intentos fallidos: " + (resp + 1) + "", "warning");
                }
            })
...
function Registrar_Usuario() {
    ...
    if (validaremail == "incorrecto") {
        return Swal.fire("Mensaje De Advertencia", "El email ingresado es incorrecto, ingrrese uno valido", "warning");
    }
...
function Modificar_Usuario() {
    ...
    if (validaremail == "incorrecto") {
        Swal.fire("Mensaje De Advertencia", "El formato de email es incorrecto, ingrese un formato valido", "warning");
    }
...
function Editar_Contra() {
    ...
    if (contranu != contrare) {
        return Swal.fire("Mensaje De Advertencia", "Las claves deben coincidir", "warning");
    }
            ...
            if (resp == 1) {
                ...
                Swal.fire("Mensaje De Confirmacion", "Contrase\u00f1a actualizada correctamente", "success")
                    .then((value) => {
                        ...
                        table.ajax.reload();
                        //Actualizar los nuevos datos que debe traer  
                        TraerDatosUsuario();
                    });
            } else {
                Swal.fire("Mensaje De Error", "La contra\u00f1a actual no coincide", "error");

            }

        } else {
            return Swal.fire("Mensaje De Error", "No se pudo actualizar la contra\u00f1a", "error");
        }
    })
...
function Restablecer_Contra() {
    ...
    $.ajax({
        ...
        if (resp > 0) {
            ...
            if (resp == 1) {
                //Swal.fire("Mensaje De Confirmacion","Su contrase&#241;a fue restablecida, revise su correo:"+email+".","success");
                Swal.fire("Mensaje De Error", "No se pudo restablecer su contrase&#241;a", "error");
            } else {
                Swal.fire("Mensaje De Advertencia", "El correo:" + email + " no se encuentra registrado.", "error");
            }
        } else {
            //Swal.fire("Mensaje De Error","No se pudo restablecer su contrase&#241;a","error");
            Swal.fire("Mensaje De Confirmacion", "Su contrase&#241;a fue restablecida, revise su correo:" + email + ".", "success");
